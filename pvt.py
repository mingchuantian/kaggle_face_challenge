...
class PatchEmbed(nn.Module):
    ...
    @get_local('input_before_embed')
    def forward(self, x):
        input_before_embed = x 
        B, C, H, W = x.shape
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=2, embed_dims=[64, 128, 256, 512],
        ...
        self.mcp=AddMarginProduct(1024,num_classes)
        self.fc1 = nn.Linear(1, 500)   # 2x512 if use concate()
        #self.fc1 = nn.Linear(2*1000, 500)
        self.fc2 = nn.Linear(500, 500)
        self.fc3 = nn.Linear(500, 2)   #used to use this for all experiments before BCEloss
        ...
        for i in range(num_stages):
            ...
            #pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dims[i]))   # changed for visualization
    ...
    def forward(self, x1, x2, target=None):
        ...
        output=self.mcp(output,target_long)
        '''
